...
public class TestContainerTrino {

    @Container
    @ServiceConnection
    static EnhancedTrinoContainer trino = new EnhancedTrinoContainer();

    private final HikariDataSource dataSource;

    public TestContainerTrino(@Autowired HikariDataSource dataSource) {
    ...
    @Test
    @Disabled("No nodes available to run query")
    public void performTestQuery() throws SQLException {
        try (Connection conn = dataSource.getConnection();
             Statement stmt = conn.createStatement();
             ResultSet rs = stmt.executeQuery(trino.getTestQueryString())) {
            assertTrue(rs.next(), "failed to get response");
            // skip check since it's not predictable
            // assertEquals(25, rs.getInt(1), "got unexpected result");
        }
    }
}
