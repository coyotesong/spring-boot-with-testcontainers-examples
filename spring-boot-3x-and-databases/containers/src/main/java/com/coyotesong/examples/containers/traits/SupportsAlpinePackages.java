 ...
 *
 *  - We can't use absolute paths since some distros use /usr/bin/apk, some use /usr/sbin/apk, etc.
 *
 * - More complex tasks should be handled by `ansible` playbooks. We can't rely on ansible
 *   being installed on the host OS so we should install it on the container (`apk add ansible`),
 *   copy the playbook(s) to the container, and then run ansible locally.
 */
@SuppressWarnings("JavadocBlankLines")
public interface SupportsAlpinePackages {
     ...
     */
    default Optional<ExecResult> updateAlpinePackages(Container<?> container) throws IOException {
        final ExecConfig config = ExecConfig.builder()
                .command(new String[] { "apk", "update", "--no-progress" })
                .user("root")
                .build();
     ...
     * @throws IOException an error occurred when communicating with the container
     */
    default Optional<ExecResult> upgradeAlpinePackages(Container<?> container) throws IOException {
        final ExecConfig config = ExecConfig.builder()
                .command(new String[] { "apk", "upgrade", "--no-interactive", "--no-progress" })
                .user("root")
                .build();
     ...
     * @throws IOException an error occurred when communicating with the container
     */
    default Optional<ExecResult> installAlpinePackages(Container<?> container, String... packageNames) throws IOException {
